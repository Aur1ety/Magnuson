...
# ASPEX-SWIS L2_BLK variable names (verify against actual CDF headers)
CDF_VARMAP = {
    "epoch":    "Epoch",
    "vsw":      "Proton_Speed",        # Solar wind speed [km/s]
    "np":       "Proton_Density",       # Proton number density [cm^-3]
    "tp":       "Proton_Temperature",   # Proton temperature [K]
    "he_flux":  "He_Flux",              # Alpha particle flux (for He/H ratio)
    "h_flux":   "H_Flux",               # Proton flux
    "vth":      "Proton_Thermal_Speed", # Thermal speed [km/s]  (optional)
}
...
def parse_cdf(filepath: Path) -> pd.DataFrame:
    ...
    data["time"] = cdflib.cdfepoch.to_datetime(raw_epoch, to_np=True)
...
def clean_and_impute(df: pd.DataFrame, max_gap_steps: int = 12) -> pd.DataFrame:
    ...
    # Short-gap fill
    df = df.fillna(method="ffill", limit=max_gap_steps)
    df = df.fillna(method="bfill", limit=max_gap_steps)
...
if __name__ == "__main__":
    # Example: run from terminal for a quick sanity check
    splits = colab_demo_pipeline()
